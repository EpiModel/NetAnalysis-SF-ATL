...
sbatch -p ckpt -A csde-ckpt --array=302-400 --export=ALL,CITY=atl,NET=main,INT=1 runsim.tsna.sh
...
sbatch -p ckpt -A csde-ckpt --array=286 --export=ALL,CITY=atl,NET=all,INT=1 runsim.tsna.sh
...
# Receive
scp hyak:/gscratch/csde/sjenness/netparam/data/sfo.*.rda data/
scp mox:/gscratch/csde/sjenness/netparam/data/atl.*.rda data/
